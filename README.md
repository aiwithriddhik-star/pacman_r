...
## Known limitation: greedy heuristic vs. real pathfinding

Manhattan distance only measures straight-line ("as the crow flies") distance — it has no idea what's actually between the ghost and the target. Near dense clusters of walls, this causes a specific problem: a neighboring tile can look like the right choice because it's numerically closer to the target, even though it's actually inside a small pocket or dead-end formed by the surrounding obstacles. The ghost commits to it, then finds itself boxed in with no good options, and ends up making a visibly "wrong" or confused-looking move right next to a wall cluster.

Why not just use BFS/A* instead? Real pathfinding would fix this — it finds the true shortest walkable route, not just the straight-line distance. The problem is when you'd need to run it: the target tile is the player's live position, which changes every time Pac-Man moves. That means the path would need to be recalculated on essentially every step, for all four ghosts, every frame. A full grid search that often is noticeably more expensive than a cheap Manhattan-distance comparison done only at intersections, and at 40 FPS that cost adds up fast.

So the current design is a deliberate trade-off: cheap, fast, "good enough" decisions at every intersection, in exchange for occasional locally-wrong moves near tricky wall clusters — rather than a fully accurate but much heavier recalculation on every single frame.
## Tech stack
Python 3
Pygame
